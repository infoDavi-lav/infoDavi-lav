# 🚚 Import/Export & Backup — Estratti

Add-on import/export con fallback ZIP, backup giornaliero e migrazione DB.

---

## 🔧 Auto-iniettore Import/Export (i18n-safe)
- Ripulisce vecchi blocchi e reinserisce HTML/JS localizzati una sola volta, gestendo la variante con calendario installato.
```php
...
## 🧭 Builder CSV/JSON per backup multiformato
- Genera CSV con header dinamici (liste, mappe, calendario) e BOM UTF-8 per compatibilità Excel.
```php
...
## 📦 Fallback ZIP streaming senza ZipArchive
- Costruisce lo ZIP a mano (LFH + Central Directory) per ambienti senza estensione ZipArchive.
```php
class SimpleZip {
...
## 🗄️ Bootstrap SQLite con migrazione da JSON legacy
- Popola il DB vuoto importando menu, coperto, tavoli, ordini e carrelli dai vecchi file.
```php
...
## 🧹 Pulizia backup vecchi con log
- Cancella dump oltre N giorni e registra `removed/kept` nei log.
```php
